...
public class KeyboardFactory implements Keyboard {
    ...
    public void createKeyboard(KeyboardType keyboardType, Update update) {
        switch (keyboardType) {
            case NEW_GAME -> keyboardSender.sendKeyboard((Keyboard) NewGameKeyboardCreator.newGameKeyboardCreate(), update);

            case QUESTIONS_MENU -> keyboardSender.sendKeyboard((Keyboard) QuestionKeyboardCreator.createQuestionKeyboard(), update);

            case MAIN_ADMIN -> keyboardSender.sendKeyboard((Keyboard) MainKeyboardCreator.MainKeyboardCreate(), update);

            default -> keyboardSender.sendKeyboard((Keyboard) DefaultKeyboardCreator.createDefaultKeyboard(), update);
